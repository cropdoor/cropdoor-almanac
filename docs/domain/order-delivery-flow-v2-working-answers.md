...
| --- | --- | --- |
| **S1** — is the inspection of the packed crate, at the gate, once per farm? | **Answered 9 Sep: no.** The order visit is an *availability check before a crew is committed* — the produce may still be in the ground. It is distinct from the farm-verification visit at registration, and distinct from the handoff, which counts what went in the van. Defined in [The order availability check](order-availability-check.md). | Steps 3/4 and 6 stay separate, deliberately. Question 10 (a farmer code at the gate) becomes more important, not less: the check is the promise, the handoff is the count. **Ops' seven answers (9 Sep)** made the check a *collection-day* act: READY triggers it, it is valid 8 working hours, one visit per farm per collection day, the delivery agent is the fallback — see questions 2, 4, 26, 34 |
...
| --- | --- | --- | --- | --- | --- | --- |
| 3 | If the agent is at the farm anyway to inspect, why not make that visit the collection? What is gained by two trips? | POLICY | S1 (after 1, 2, 9) | **Collapse them** if 1 = packed crate. One farm-gate moment: farmer presents, agent inspects and receives, both sign. If 1 = standing crop, keep the early visit *but* as a per-farm-per-day act, not per order — and accept that the crate is inspected again at handoff. | **Two trips are deliberate — but on the same day.** What is gained: a crew never *collects* produce nobody has seen. With the 8-working-hour validity (question 26) the check is the agent's morning round and the collection follows it; where there is no field agent, the delivery agent makes both in one visit (question 4). | Keep step 3 and step 6 as separate acts, even when one person does both on one visit. Accept a light count at handoff |
...
| **34** | **What is the cancellation rule while the order waits for the field agent — between "accepted" and "READY"?** | POLICY | S1 | *Not asked before.* The flow says free until accepted, penalty at READY; the wait in between — the longest window — is unspecified. **Revised proposal (9 Sep):** the buyer may cancel **free until the farmer marks READY**; the penalty window opens at READY — which is where the team's flow already put it. The earlier proposal ("free until Confirmed") no longer works: with an 8-working-hour validity the check is on collection morning, and a free window that long would let a buyer cancel after the farmer has harvested and packed. Acceptance alone still does not open the penalty — the farmer has committed nothing yet. | — *(revised proposal awaiting leadership)* | If accepted: the flow's "free until step 2" becomes "free until READY (step 4)" |
...
| 18 | If a farmer accepts and then cancels or fails to hand over, does the farmer pay anything? Does the buyer get anything for the wait? | POLICY | — | The flow's penalties are one-sided. Engineering's view: at minimum the buyer is refunded in full *including the delivery fee*, and a farmer failure after READY is recorded against the farm. **Sharper after 9 Sep:** because the check is on collection morning, a farmer who marks READY on produce that is not there has already cost us a crew's trip to the zone. | — | — |
...
| 26 | Once READY, how quickly must the van collect? Is produce saleable after that? | POLICY | S1 | Sets the collection SLA — and whether a buyer whose order sits READY waiting for *our* van is still in the penalty window. Engineering's view: they should not be. | **Partly (9 Sep):** a Confirmed check is valid **8 working hours**, so check and collection fall on the same working day. Still open: how many days may an order sit READY before its collection day, and is the buyer still in the penalty window while it waits for our van | Collection is planned for the day of the check; a crew delayed past the window repeats the check before collecting |
