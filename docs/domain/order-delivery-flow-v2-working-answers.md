...
| 14 | Nobody home, or the buyer refuses the crate: retry (who pays), redirect, return to farm, write off? Who decides — agent, supervisor, office? | POLICY | S2 | This is the missing **rejection path at the door**. It needs a named outcome, an owner, and an answer to "is the farmer paid" — they did everything right. **After 13 (10 Sep):** the *mechanics* are settled — the agent does not hand over, records DELIVERY FAILED with the reason, the produce goes back to the farm. What stays open here is the policy: does the buyer get a second attempt, who pays for it, and what the farmer gets. | — *(mechanics settled; policy open)* | — |
...
| 20 | When an agent collects cash, when and how does it reach us? How do we know at month-end which cash is still in agents' pockets? | FACT | — | Today cash is recorded as ours the moment the agent confirms. Whatever the real remittance is (daily to office, mobile money, bank) becomes a step with a record, so "collected" and "banked" are different facts. **After 32 (10 Sep):** agents are employees, so this is ordinary staff cash handling — the question is only *when and how* (end of day to the office? mobile money?). | — | — |
