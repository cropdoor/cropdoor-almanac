...
| 10 | Should the farmer have their own code — read out to the crew — so a disagreement about what was handed over has two records, not one? | POLICY | S3 | **Yes.** The buyer-door code solves exactly this problem at the other end. A farmer code sent by SMS works on any phone and turns the agent's tap into a two-party record. **After S3 (10 Sep):** the farmer's own HANDOFF tap already gives the second record, so the code is only needed where the farmer cannot tap (question 7). | — *(now optional; hangs on 7)* | — |
