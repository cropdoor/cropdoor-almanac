...
!!! info "Superseded as the description of intent"
    The team's 7 September flow is the design now — drawn, with what has been settled since, on
    [The order → delivery flow, v2](order-delivery-flow-v2.md); the [gap analysis](order-delivery-flow-v2-gap-analysis.md)
    is where its questions came from. This page stays as the record of what exists
