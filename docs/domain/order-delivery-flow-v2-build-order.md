...
| 3 · Money rails | T1 | the agent's collected cash recorded as received, so a cash order can be paid at all | Not started | — |
| 3 · Money rails | H | the payout run reads the ledger and skips a disputed order | In review | #276 |
...
Seventeen of thirty-seven are merged: **steps 0 and 1 are complete**, and step 2 is three PRs in. **#232 and #243 are not among them:** #232 corrected a Javadoc about the buyer's cancellation window, found while building step 0, and #243 lowered how many messages the dispatcher sends at once, decided while reading X2b's live run.
